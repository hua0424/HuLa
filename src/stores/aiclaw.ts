 ...
 */
export const useAiclawStore = defineStore(
  StoresEnum.AICLAW,
  () => {
    ...
    const ensureLoaded = async () => {
      ...
      try {
        ...
        console.error('[AiclawStore] ensureLoaded failed:', err)
        if (callGen !== generation) {
          return
        }
      } finally {
    ...
    const isMyAiclaw = (uid: string | number | undefined | null): boolean => {
      ...
      const set = myAiclawUids.value
      if (set instanceof Set) {
        return set.has(String(uid))
      }
      if (Array.isArray(set)) {
        return set.includes(String(uid))
      }
      return false
    }
  ...
  {
    // 该 store 是内存缓存，持久化会把 Set 序列化成数组，导致 isMyAiclaw 运行时异常
    persist: false
  }
)
